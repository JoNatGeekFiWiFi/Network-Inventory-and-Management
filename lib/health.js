...
export const METRICS = {
  ...
  // Resources: read every five minutes. Defaults as OpenWISP ships them.
  cpu: {
    label: 'CPU', unit: '%', bool: false, critical: false,
    help: 'CPU load (RouterOS reports it directly; on OpenWrt it is the load average over the cores).',
    rule: { op: '>', threshold: 90, tolerance_min: 30 },
    alert: 'has high CPU load', recovery: 'CPU load is back to normal'
  },
  memory: {
    label: 'Memory', unit: '%', bool: false, critical: false,
    help: 'Memory in use, not counting cache the kernel can give back.',
    rule: { op: '>', threshold: 95, tolerance_min: 30 },
    alert: 'is almost out of memory', recovery: 'memory use is back to normal'
  },
  disk: {
    label: 'Storage', unit: '%', bool: false, critical: false,
    help: 'Flash storage in use. A full flash stops configuration changes and backups from saving.',
    rule: { op: '>', threshold: 90, tolerance_min: 0 },
    alert: 'is almost out of storage', recovery: 'storage use is back to normal'
  },
  config: {
    label: 'Configuration', unit: '', bool: true, critical: false,
    help: 'Whether the router still matches its configuration templates. Checked hourly.',
    rule: { op: '<', threshold: 1, tolerance_min: 0 },
    alert: 'no longer matches its configuration templates', recovery: 'matches its configuration templates again'
  }
};
