...
export async function probeDevice(target, deps) {
  ...
  if (openPorts.includes(22) && sshExec) {
    ...
    if (r.ok) {
      ...
      if (j) {
        ...
        add('ubus over SSH', true, `works as ${user} — ${board.model || 'unknown model'}, ${board.osVersion || 'unknown build'}`, { board });
  ...
  return {
    ...
    apply: suggested
      ? { platform: suggested, mgmt_transport: ubusHttp ? 'http' : (ssh ? 'ssh' : 'auto') }
      : null
  };
}
