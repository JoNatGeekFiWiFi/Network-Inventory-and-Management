...
function run(bin, args, { sudo = true, timeoutMs = 8000 } = {}) {
  return new Promise((resolve) => {
    // The demo has no WireGuard hub to drive, and must not touch this server's real one.
    if (process.env.DEMO_MODE === '1') return resolve({ ok: false, stdout: '', error: 'Demo mode: the WireGuard hub is not available in the demo.' });
