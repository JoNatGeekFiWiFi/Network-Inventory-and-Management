 ...
 */
// Windows-1252 bytes that WinAnsi uses for characters outside Latin-1.
const WIN1252 = new Map([
  ['\u20AC', 0x80], ['\u201A', 0x82], ['\u0192', 0x83], ['\u201E', 0x84], ['\u2020', 0x86],
  ['\u2021', 0x87], ['\u02C6', 0x88], ['\u2030', 0x89], ['\u0160', 0x8A], ['\u2039', 0x8B],
  ['\u0152', 0x8C], ['\u017D', 0x8E], ['\u02DC', 0x98], ['\u2122', 0x99], ['\u0161', 0x9A],
...
export function encodeText(s) {
  ...
  for (const ch of String(s)) {
    ...
    for (const c of folded) {
      ...
      if (c === '\\' || c === '(' || c === ')') out += '\\' + c;
      else if ((code >= 32 && code <= 126) || (code >= 160 && code <= 255)) out += c;
      else if (WIN1252.has(c)) out += String.fromCharCode(WIN1252.get(c));
      else out += '?';
    }
  }
  return out;
...
export function createPdf({ title = '', author = '', compress = true } = {}) {
  ...
  const api = {
    ...
    strokes(strokeList, { x = 72, top = 72, width = 200, height = 60, colour = [0, 0, 0.55], lineWidth = 1.6 } = {}) {
      ...
      // A quadratic through the pen samples, emitted as a cubic because that is the curve PDF has.
      // Straight segments make a quick signature look jagged once it is printed.
      const cubic = (start, ctrl, end) => {
        const c1x = start.x + (2 / 3) * (ctrl.x - start.x);
        const c1y = start.y + (2 / 3) * (ctrl.y - start.y);
        const c2x = end.x + (2 / 3) * (ctrl.x - end.x);
        const c2y = end.y + (2 / 3) * (ctrl.y - end.y);
        parts.push(`${n(c1x)} ${n(c1y)} ${n(c2x)} ${n(c2y)} ${n(end.x)} ${n(end.y)} c`);
      };
      for (const stroke of strokeList) {
        ...
        const mapped = stroke.map(p => ({ x: px(p), y: py(p) }));
        parts.push(`${mapped[0].x} ${mapped[0].y} m`);
        if (mapped.length === 2) {
          parts.push(`${mapped[1].x} ${mapped[1].y} l S`);
          continue;
        }
        for (let i = 1; i < mapped.length - 1; i++) {
          const end = i === mapped.length - 2
            ? mapped[mapped.length - 1]
            : { x: (mapped[i].x + mapped[i + 1].x) / 2, y: (mapped[i].y + mapped[i + 1].y) / 2 };
          const start = i === 1 ? mapped[0] : {
            x: (mapped[i - 1].x + mapped[i].x) / 2,
            y: (mapped[i - 1].y + mapped[i].y) / 2
          };
          cubic(start, mapped[i], end);
        }
        parts.push('S');
      }
      parts.push('Q');
    ...
    /**
     * A table. `columns` is {label, weight, align} and each row is an array of cell strings.
     * Returns the top coordinate just under the last row. Starts a new page rather than drawing
     * through the footer.
     */
    table(rows, { x = 72, top = 72, width = 468, columns, size = 9, bottom = 736, header = true } = {}) {
      ...
      const drawRow = (cells, bold) => {
        ...
        cells.forEach((cell, i) => {
          ...
          const tw = textWidth(str, font, size);
          const align = columns[i].align || 'left';
          const tx = align === 'right' ? cx + widths[i] - tw - 4 : align === 'center' ? cx + (widths[i] - tw) / 2 : cx + 2;
          api.text(str, { x: tx, top: top + size, font, size });
          cx += widths[i];
        });
        api.line(x, top + rowH - 3, x + width, top + rowH - 3, { colour: [0.82, 0.82, 0.82], width: 0.4 });
        top += rowH;
      };
      const drawHeader = () => drawRow(columns.map(c => c.label), true);
      if (header) drawHeader();
      for (const row of rows) {
        if (top + rowH > bottom) { api.addPage(); top = 72; if (header) drawHeader(); }
        drawRow(row, false);
      }
      return top;
    },

    /** Run once per page at build time. Used for a footer that has to know the page count. */
    decorate(fn) { decorators.push(fn); return api; },

    /** Serialise. Returns a Buffer. */
    build() {
      if (decorators.length) {
        const saved = current;
        pages.forEach((p, i) => { current = p; for (const fn of decorators) fn(i, pages.length); });
        current = saved;
      }
