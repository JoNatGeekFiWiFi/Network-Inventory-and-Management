...
const KIND_MAP = {
  ...
  // Rack-mounted gear inside a POP or building. myWorld models these as their own tables and
  // uses them as span endpoints; none of them is a buried chamber, so without these the default
  // silently files thousands of equipment terminations as 'handhole'.
  shelf: 'cabinet', rack: 'cabinet', slot: 'cabinet', wall_box: 'cabinet',
  splice_shelf: 'splice_case', mywcom_fiber_slack: 'splice_case',
  // A span that lands mid-cable rather than on a structure — the join is a splice.
  mywcom_fiber_segment: 'splice_case', mywcom_route_junction: 'handhole',
  access_point: 'handhole', floor: 'building', room: 'building'
};
    ...
    if (p.user_count != null || String(p.user_type || '').toLowerCase() === 'span') {
      ...
      if (coords && coords.length >= 2) {
        ...
        out.routes.push({
          ...
          coordinates: coords,
          // IQGeo's figure is the MEASURED FIBRE distance, which is longer than the ground path
          // because of slack. Keeping it separate lets the locator derive real slack per route
          // instead of assuming a flat percentage; length_m stays null so ground is computed
          // from the geometry itself.
          length_m: null,
          fibre_m: numOr(p.user_actual_fiber_dist_km_sfdc) != null ? Math.round(numOr(p.user_actual_fiber_dist_km_sfdc) * 1000) : null,
