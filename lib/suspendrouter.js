...
// ---- one call per platform, used by the app and by tools/device-check.mjs ------------------------

/**
 * Put a RouterOS router into (or out of) the suspended state. `call(method, path, body)` talks REST.
 * Returns a short description of what changed; throws with a readable message when it cannot.
 */
export async function enforceRouterOS(call, { suspend, captiveIp, captivePort, garden, protect = [] }) {
  if (!suspend) { const n = await routerosClear(call); return n ? `removed ${n} rule(s)` : 'nothing to remove'; }
  await routerosSuspend(call, routerosPlan({ captiveIp, captivePort, garden }), { protect });
...
/**
 * The same for OpenWrt, through the driver's UCI calls. Staged, then applied with an automatic
 * rollback: if the change cut us off, the router undoes it by itself within the timeout.
 */
export async function enforceOpenWrt(driver, { suspend, mgmtAddress, captiveIp, captivePort, gardenIps, timeoutSeconds = 60 }) {
  if (!driver.uciGetAll) throw new Error("This device's driver cannot change the firewall");
  // `uci apply` applies EVERY staged change on the router, not just ours. If someone left edits
  // half-made in LuCI, they would go live with the suspension — refuse rather than do that.
  const pending = driver.uciChanges ? await driver.uciChanges() : [];
  if (pending.length) throw new Error(`The router has ${pending.length} unsaved configuration change(s) from elsewhere (LuCI?). Apply or revert them first.`);
  ...
  if (suspend) {
    ...
    const zone = findOverlayZone({ interfaces: ifs, firewall: fw.values, mgmtAddress });
    ...
    for (const sec of openwrtPlan({ captiveIp, captivePort, gardenIps, overlayZone: zone })) {
      const r = await driver.uciAdd('firewall', sec.type, sec.name, sec.values);
      if (!r.ok) { await driver.uciRevert('firewall'); throw new Error(`Could not stage ${sec.name}: ${r.error}`); }
    }
  } else if (!ours.length) return 'nothing to remove';
  const applied = await driver.applyConfirmed({ timeoutSeconds });
  if (!applied.ok) throw new Error(applied.error);
  return suspend ? 'suspension rules on' : `removed ${ours.length} section(s)`;
}

/** Is an OpenWrt router currently carrying our suspension sections? */
export async function openwrtIsSuspended(driver) {
  const fw = await driver.uciGetAll('firewall');
  if (!fw.ok) throw new Error('Could not read the firewall: ' + fw.error);
  return openwrtOurs(fw.values).includes(OW_PREFIX + 'block');
}
