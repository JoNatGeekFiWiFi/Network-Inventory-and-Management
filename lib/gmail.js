...
export function normaliseMessage(message) {
  ...
  return {
    ...
    // Plus-addressing often survives only on a delivery header. The To line the sender typed can be
    // rewritten to the bare mailbox, and the token we need is then only on Delivered-To.
    routedTo: addresses([
      header(message, 'To'),
      header(message, 'Cc'),
      header(message, 'Delivered-To'),
      header(message, 'X-Original-To'),
      header(message, 'X-Gm-Original-To')
    ].filter(Boolean).join(', ')),
...
/**
 * Does this message carry a thread token we issued, or a ticket number?
 *
 * A support inbox also receives newsletters, carriers, and mail that is not a customer reply.
 * Feeding all of it into the timeline would open a ticket for every one of them.
 */
export function replyTokenIn(msg) {
  const blob = [...(msg && msg.routedTo || []), ...(msg && msg.to || []), ...(msg && msg.cc || []), (msg && msg.subject) || ''].join(' ');
  return /\+[0-9a-f]{24,}@/i.test(blob) || /TKT-\d+/i.test((msg && msg.subject) || '');
}

/**
 * The new text of a reply, without the quoted original underneath it.
 *
 * Gmail and Outlook both append the message being answered. Stored whole, the timeline repeats
 * our own outbound text under every reply and the customer's line is buried at the top of a page.
 */
export function visibleReply(text) {
  ...
  if (quoted > 0) cut = Math.min(cut, quoted);
