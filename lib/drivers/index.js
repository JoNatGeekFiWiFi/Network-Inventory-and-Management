...
export const CAPABILITIES = [
  ...
  'packages'         // install / remove software packages (opkg)
];
...
export const PLATFORMS = {
  ...
  openwrt: {
    ...
    // Monitoring, plus the first write. WiFi is deliberately the only write so far: it cannot
    // sever the management path, which rides on the WAN, so it proves the confirmed-apply loop on
    // a change that cannot lock anyone out.
    //
    // configBackup came FIRST — before any write existed — because a write path without an undo is
    // not a feature, it is a liability.
    //
    // Absent on purpose: `firmware`. system.sysupgrade is present on this hardware and is not
    // wired up. A firmware push that fails at a customer's house is not recoverable remotely, and
    // it should arrive with its own testing rather than beside a WiFi rename.
    // Writes go through UCI with confirmed apply (the router rolls back on its own if we lose it);
    // firmware and packages need SSH. See lib/drivers/openwrt-write.js.
    caps: ['identity', 'interfaces', 'traffic', 'dhcpRead', 'dhcpWrite', 'wifiRead', 'wifiWrite', 'wifiClients',
           'logRead', 'blocklistPush', 'configBackup', 'firmware', 'reboot', 'wireguardPush', 'packages']
  },
  ddwrt: {
    label: 'DD-WRT',
    short: 'DD-WRT',
