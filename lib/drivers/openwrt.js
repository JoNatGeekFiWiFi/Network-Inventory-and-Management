...
import { extendWrites } from './openwrt-write.js';
  ...
  for (const [radio, r] of Object.entries(d || {})) {
    ...
    for (const i of r.interfaces || []) {
      ...
      out.push({
        iface: c.ifname,
        radio,
        // The UCI section name (wifi2g, default_radio0 …). Writes address a network by it; dropping
        // it here left the Wi-Fi edit form with nothing to send.
...
export function sshTransport({ host, username, password, port = 22, timeoutMs = 12000, sshExec }) {
  return {
     ...
     */
    async run(argv, opts = {}) {
      const r = await sshExec({ host, username, password, port, timeoutMs: opts.timeoutMs || timeoutMs, argv, stdin: opts.stdin || null });
...
export function autoTransport(mk) {
  ...
  return {
    ...
    async run(argv, opts) {
      ...
      return t.run ? t.run(argv, opts) : { ok: false, error: 'no shell available on this transport' };
    }
  };
}
...
export function createDriver(device, deps = {}) {
  ...
  const driver = {
    ...
    async wifi() {
      ...
      for (const radio of all) {
        ...
        if (!c) continue;
        if (c.section) radio.section = c.section;
    ...
    /** Stage removal of one section (or one option of it). */
    async uciDelete(config, section, option = null) {
      if (!isUciName(config) || !isUciName(section) || (option != null && !isUciName(option))) return { ok: false, error: 'Invalid config, section or option name' };
      const r = await transport.call('uci', 'delete', option ? { config, section, option } : { config, section });
      return r.ok ? { ok: true } : { ok: false, error: r.error };
    },
  ...
  // Writes that go beyond Wi-Fi — blocklist, DHCP, reboot, firmware, packages, WireGuard — live in
  // their own module so this file stays about reading the device.
  return extendWrites(driver, transport);
}
