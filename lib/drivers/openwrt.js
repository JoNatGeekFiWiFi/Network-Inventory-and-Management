...
export function sshTransport({ host, username, password, port = 22, timeoutMs = 12000, sshExec }) {
  return {
    ...
    /** For things with no ubus object, like logread. */
    async run(argv) {
      const r = await sshExec({ host, username, password, port, timeoutMs, argv });
      return r.ok ? { ok: true, data: r.stdout } : { ok: false, error: r.error };
    }
  };
}
...
export function createDriver(device, deps = {}) {
  ...
  return {
    ...
    async interfaces() {
      ...
      return { interfaces: mergeInterfaces(devs, logical), logical };
    },
