...
export function explainTokenError(status, body) {
  ...
  if (both.includes('invalid_grant') && both.includes('account not found')) {
    return 'That mailbox does not exist as a user in the domain. If it is a Google Group, delegation cannot reach it — a group has no Gmail mailbox. It needs to be a real user account.';
  }
...
export function createTokenSource({ key, fetchImpl = fetch, now = () => Date.now() }) {
  ...
  async function token(subject, scopes) {
    ...
    const res = await fetchImpl(key.token_uri || TOKEN_URL, {
      method: 'POST',
